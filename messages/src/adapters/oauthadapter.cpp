...
void OauthAdapter::send(const std::string& message, const std::string& adress) {
  ...
  auto encoded = base64_encode(
      reinterpret_cast<const unsigned char*>(message.c_str()), static_cast<unsigned int>(message.size()));
...
bool OauthAdapter::connect(const ConnectionHolder& conn) {
    switch (conn.getType()) {
      ...
      default:
        throw std::runtime_error("Undefined connection type: " + std::to_string(static_cast<int>(conn.getType())));
    }
}
