...
void UdpAdapter::send(const std::string& message, const std::string& adress) {
  using asio::ip::udp;
  udp::endpoint remote_endpoint(asio::ip::address_v4::from_string(adress),
                                5555);
  ...
  socket.send_to(asio::const_buffers_1(&message[0], message.size()),
                 remote_endpoint);
}
...
std::pair<std::string, std::string> UdpAdapter::receive() {
  ...
  if (mSocket) {
    ...
    mSocket->receive_from(asio::buffer(&messg[0], messg.size()),
                          remote_endpoint);
