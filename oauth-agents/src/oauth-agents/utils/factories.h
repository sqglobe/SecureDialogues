...
public:
    virtual std::unique_ptr<ApiAgent> makeApiAgent(const std::string& clientId) const = 0;
    virtual std::unique_ptr<OAuthAgent> makeOauthAgent() const = 0;
    virtual std::unique_ptr<OAuthAgent> makeOauthAgent(const std::string& refreshToken) const = 0;
